...
class EngineVecEnv:
    def __init__(self, num_envs: int, seed: int = 0, max_turns: int = 300,
        ...
        # Potential-based reward shaping: r += shaping_coef * (gamma * Φ(s') - Φ(s)), with
        # Φ = learner-relative team-HP differential. Policy-invariant (can't bias the policy);
        # 0.0 = off (pure sparse reward).
    ...
    def step(self, learner_actions, opponent_actions):
        """Advance every battle. Returns (reward[N], done[N], dyn[N,4], outcome[N]):
          reward  — training reward (sparse ±1/0, plus PBRS shaping if shaping_coef > 0)
          dyn     — world-model labels [hp_Δ_self, hp_Δ_opp, ko_self, ko_opp] (learner-relative)
          outcome — the *true* win/loss (+1/-1/0), for unbiased win-rate logging (= reward if no shaping)
        """
        rewards = np.zeros(self.num_envs, dtype=np.float32)
        dones = np.zeros(self.num_envs, dtype=np.float32)
        dyn = np.zeros((self.num_envs, 4), dtype=np.float32)
        outcome = np.zeros(self.num_envs, dtype=np.float32)
        shaping = self.shaping_coef > 0.0
        for i, b in enumerate(self.battles):
            ls = int(self.learner_side[i])
            opp_s = 1 - ls
            # Pre-turn HP of each side's active (for dyn), and team-HP potential Φ (for shaping).
            ai = (b.active_index(0), b.active_index(1))
            pre = (b.hp_fraction(0, ai[0]), b.hp_fraction(1, ai[1]))
            phi_pre = (b.team_hp_fraction(ls) - b.team_hp_fraction(opp_s)) if shaping else 0.0
            ...
            dyn[i, 0] = post[ls] - pre[ls]
            dyn[i, 1] = post[opp_s] - pre[opp_s]
            dyn[i, 2] = 1.0 if pre[ls] > 0 and post[ls] <= 0 else 0.0
            dyn[i, 3] = 1.0 if pre[opp_s] > 0 and post[opp_s] <= 0 else 0.0

            self._ep_len[i] += 1
            if not done and self._ep_len[i] >= self.max_turns:
                done, winner = True, -1  # timeout -> draw

            true_r = (1.0 if winner == ls else (-1.0 if winner == (1 - ls) else 0.0)) if done else 0.0
            outcome[i] = true_r
            r = true_r
            if shaping:
                # PBRS: terminal state has potential 0 (preserves policy invariance).
                phi_post = 0.0 if done else (b.team_hp_fraction(ls) - b.team_hp_fraction(opp_s))
                r += self.shaping_coef * (self.gamma * phi_post - phi_pre)
            rewards[i] = r
            ...
            if done:
                ...
                self._ep_len[i] = 0
                self.learner_side[i] = self._rng.integers(0, 2)
        return rewards, dones, dyn, outcome
