...
| **scale2** (Rust-env era begins, 2026-07-27; ongoing) | `train_flow` on `FlowVec` decision points: 13 actions, real PS team pool, league PFSP (snapshots + Rust heuristic w4.0), Φ=HP+0.5·faints shaping 0.15, 1024×3 trunk, TF32+fused Adam, ~16k sps | 0.083@3.4M → 0.121@13M → **0.238@125M** vs heuristic (rising; scale1's asymptote 0.185 broken) | throughput campaign: Rust heuristic port (was 83% of wall time), TF32, fused Adam, crash-proofed pause-resume; entropy 1.0@130M healthy vs scale1 collapse. NOTE: this era's heuristic eval is the FIXED port — numbers are not directly comparable to poke-env-era heuristic rows |
